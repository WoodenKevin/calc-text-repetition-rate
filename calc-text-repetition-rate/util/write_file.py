...
class WriteFile(object):
    ...
    def write_basic_info(self, org_file_path, cmp_file_path):
        ...
        self._file_obj.write('原始文件路径： {}\n\n'.format(
            os.path.abspath(org_file_path)))

        self._file_obj.write('待比较文件名： {}\n'.format(
            os.path.basename(cmp_file_path)))
        self._file_obj.write('待比较文件路径： {}\n\n\n'.format(
            os.path.abspath(cmp_file_path)))
        self._file_obj.write('【文本重复率1. 运算结果】\n')

    def write_matrix_res(self, rate, time):
        self._file_obj.write('利用二维矩阵查找连续重复的字符串实现\n')
